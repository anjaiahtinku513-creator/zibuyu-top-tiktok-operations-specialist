...
description: End-to-end Zibuyu TikTok operations workflow for apparel ecommerce videos. Use when the user provides product images, an Amazon product URL, clothing references, or a model name and wants Codex to analyze exact product attributes and direct buyer-review evidence before scriptwriting, generate white-background clothing three-view references, compile evidence-bound United States or Germany Seedance UGC prompts through distinct market profiles, preserve a natural three-phase creator performance arc and continuous phone/prop state, apply deterministic quality gates, submit PopBoom advanced video generation, report/poll tasks, distinguish platform success from creative-quality acceptance, and return video status/links with copy-ready captions and exactly 5 hashtags.
---
...
This plugin owns four bundled skills: `zibuyu-top-tiktok-operations-specialist`, `clothing-three-view`, `seedance-ugc-cn-director`, and `popboom`.
...
- Load the bundled skill files from this plugin directory first, even if the current thread's visible Skills list only exposes one of them.
- Use standalone copies under `$CODEX_HOME\skills` only as a fallback or synchronization target when the bundled copy is inaccessible.
...
- For German Zibuyu apparel delivery, hashtags must be exactly five total. Choose all five from garment, German TikTok Shop, occasion, outfit context, style, and buyer search intent; do not force a fixed brand hashtag.
...
- Whenever a bundled plugin skill is changed, mirror the same optimization into the corresponding standalone skill under `$CODEX_HOME\skills` when that standalone skill exists.
...
- For every Zibuyu apparel job, provide exactly five unique hashtags. Choose all five from product-, market-, occasion-, style-, buyer-search-, or TikTok Shop-relevant tags; do not force a fixed brand hashtag.
- If the delivery validator fails because copy is missing or invalid, route only the copy fields back through `seedance-ugc-cn-director`; do not rerender or resubmit the video.
...
The plugin workflow is not complete until the Copy Delivery Pack Gate also passes and its complete output is delivered. A response containing only PopBoom statuses, record IDs, or video links is incomplete.
